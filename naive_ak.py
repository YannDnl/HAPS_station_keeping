...
def main(HAPS, time, target, aero, n, nb_steps, p_angle = 2, pas = 500):
    ...
    for _ in range(nb_steps):
        ...
        for k in range(n):
            ...
            i = np.argmax(((np.cos(colonne_interp[:,1]*np.pi)/2+.5)**p_angle - 0.5**p_angle )*colonne_interp[:,0])
    ...
    HAPS.plot(title = f"naive_ak : distance moyenne {sum(score_malin)/nb_steps}, distance finale {score_malin[-1]}")