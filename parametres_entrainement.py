...
def position_initiale(time = None): ##renvoie une position et un instant initiale aléatoire
    ans = rd.randint(0, (365 - int(duration)) * 4)
    if time != None:
        days = (ans + time['hour']//6)//4
        i = -1
        ...
        start_time['hour'] = (ans * 6 + time['hour'])%24
        ans = start_time
    pos = []
