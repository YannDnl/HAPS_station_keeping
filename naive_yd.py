...
def main(HAPS, time, target, aero, n, nb_steps, pas = 500):
    ...
    for _ in range(nb_steps):
        ...
        for k in range(n):
            ...
            for k2fat in range(len(colonne_interp)):
                ...
                ans.append([x, np.abs(((np.angle(complex(colonne_interp[k2fat][0], colonne_interp[k2fat][1])) - angle + np.pi)%(2*np.pi) - np.pi)/np.pi)])
            colonne_interp = np.array(ans)
            i = np.argmin(colonne_interp[:, 1])
    ...
    #     for k in range(n):
    #         pos = HAPS.list_ballon[k].pos
    #         colonne = aero.get_vent(pos, time, target)[3:7, 1]
    #         i = np.argmin(colonne)
    ...
    HAPS.plot(title = f"naive_yd : distance moyenne {sum(score_malin)/nb_steps}, distance finale {score_malin[-1]}")